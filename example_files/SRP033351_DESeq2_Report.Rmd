...
HTSeq (0.6.0) function htseq-count was used to count reads. Counts for all samples were concatenated into the following text file:<br>
...
coldata <- read.table('/home/mengykan/Projects/SRP033351/files/SRP033351_Info_Sheet_index_withQC.txt', sep='\t', header=TRUE)
...
Boxplots for favorite genes

```{r, eval=T, echo=F, cache=FALSE, warning=F, message=F, fig.height=4, fig.width=4}
fav_genes <- c('ORMDL3', 'CRISPLD2', 'SOCS1', 'SOCS2', 'SOCS3', 'SOCS4', 'SOCS5', 'SOCS6','SOCS7', 'CEBPD', 'CISH', 'GUCY1A3', 'GUCY1B3', 'CCL5', 'CXCL10', 'CX3CL1', 'CCL13','IL6', 'IL8', 'HHIP', 'SPATS2L', 'CCR2', 'CCR4', 'SST', 'SSTR1', 'CCK','CCKAR','PDE8')
for (i in fav_genes) {
  gene_symbol=i
...
Differential expression results for favorite genes
...
## Housekeeping genes
Counts have been normalized by estimated size factors using DESeq2 function counts to obtain the count matrix.
...
for (i in housekeeping_genes) {
  ...
  curr_data <- norm.counts[which(norm.counts$gene_symbol==i),names(norm.counts)[names(norm.counts)%in%coldata$Sample]] # choose Ensembl gene with the most counts
  curr_data <- curr_data[which.max(rowSums(curr_data)),]
  gene_id <- gene_ids[which.max(rowSums(curr_data))]
