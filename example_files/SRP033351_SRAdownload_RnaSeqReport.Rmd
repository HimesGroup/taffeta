...
```{r var, echo=T}
out_dir <- '/home/mengykan/Projects/SRP033351/SRP033351_SRAdownload/'
...
if (exists("geo_id")){
  if (!exists("pheno_fn")) {
    ...
    if (all(c("relation","relation.1")%in%names(pheno.raw))) {
      relation=pheno.raw$relation.1
      if (all(grepl("SRX",pheno.raw$relation))) {relation=pheno.raw$relation} # if SRX ids exist in relation column (e.g. GSE40996)
    }
    files <- gsub(".*=","",relation)
  } else {
