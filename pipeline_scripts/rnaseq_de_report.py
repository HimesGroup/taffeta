...
def make_deseq2_html(rmd_template, project_name, path_start, sample_info_file, ref_genome, comp_file, template_dir):
    ...
    outp.write("\n")
    outp.write("```{r, eval=T, echo=F, cache=F, warning=F, message=F, fig.height=4, fig.width=4}\n")
    ...
    outp.write("fav_genes_notexist <- fav_genes[!fav_genes%in%norm.counts$gene_symbol]\n")
    outp.write("if (length(fav_genes_notexist)>0) {\n")
    outp.write("cat('These control genes do not exist:', paste(fav_genes_notexist, collapse=','))\n")
    outp.write("fav_genes <- fav_genes[fav_genes%in%norm.counts$gene_symbol]}\n")    
